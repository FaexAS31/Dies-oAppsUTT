...
const DesaprobadasScreen = ({ navigation }) => {
  const [solicitudesDesaprobadas, setSolicitudesDesaprobadas] = useState([]);
  const [selectedSolicitud, setSelectedSolicitud] = useState(null);
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedRow, setSelectedRow] = useState(null);

  useEffect(() => {
    obtenerSolicitudesDesaprobadas();
  }, []);

  const obtenerSolicitudesDesaprobadas = async () => {
    try {
      const response = await axios.get("http://localhost:3000/desaprobadas", {
        timeout: 5000,
      });
      setSolicitudesDesaprobadas(response.data);
    } catch (error) {
      console.error("Error al obtener las solicitudes desaprobadas:", error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Solicitudes Desaprobadas</Text>
      <ScrollView>
        {solicitudesDesaprobadas.map((solicitud, index) => (
